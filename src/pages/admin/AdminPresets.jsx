...
export default function AdminPresets() {
  ...
  async function approve(preset) {
    ...
    logActivity(user.id, { action: 'preset_approved', targetType: 'preset', targetName: preset.name })
    if (preset.created_by) {
      notifyUser(preset.created_by, {
        type: 'preset_approved',
        title: 'Preset aprobado',
        body: `"${preset.name}" xa está dispoñible para todos`,
        link: '/ferramentas',
      })
    }
    refetch()
  ...
  async function reject(preset) {
    ...
    logActivity(user.id, { action: 'preset_rejected', targetType: 'preset', targetName: preset.name })
    if (preset.created_by) {
      notifyUser(preset.created_by, {
        type: 'preset_approved',
        title: 'Preset rexeitado',
        body: `"${preset.name}" non foi aprobado`,
      })
    }
        ...
        {loading ? <p className="text-xogun-muted text-sm">Cargando...</p> :
          pending.length === 0 ? <p className="text-xogun-muted text-sm">Non hai presets pendentes. ✅</p> : (
            <div className="space-y-3">
              {pending.map(p => {
                const isScoreTemplate = p.config?.scoreSections?.length > 0
                return (
                  <div key={p.id} className="card flex items-start gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xogun-muted text-xs">
                        {p.games?.name || 'Xogo descoñecido'} · {isScoreTemplate ? 'Plantilla de marcador' : 'Preset de ferramentas'}
                      </p>
                      <div className="flex gap-1 flex-wrap mt-1.5">
                        {isScoreTemplate
                          ? p.config.scoreSections.map(s => (
                              <span key={s.id} className="badge border-xogun-accent/30 text-xogun-accent text-[10px]">{s.label}</span>
                            ))
